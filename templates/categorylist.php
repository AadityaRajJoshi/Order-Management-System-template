          ...
          <div style="height: 450px;" class="table-wrapper-scroll-y my-custom-scrollbar">
            <table  class="table table-dark table-data3">
              ...
              <tbody>
                ...
                <th>{{id}}</th>
                <td>{{name}}</td>
                <td><form><a href="javascript:App.navigate('UpdateCategory')" <button class="btn btn-primary" id="getCateogryId" data-id={{id}}>Edit</button></a>
                    <button class="btn btn-danger" id="deleteCategory" data-id={{id}}>Delete</button> </td>
                    </form> </td>
                </tr>
                {{/each}}
